...
static bool i_rationalTest_simplifyCheck(i8 sign, u32 num, u32 den,
  ...
  Natural* scratch[5] = {&scr_gcd, &scr_a, &scr_b, &scr_div, &scr_q};

  s = rational_set(alloc, sign, num, den, &rat); checkStatus(s);
  s = rational_simplify(alloc, &rat, scratch);
  checkStatus(s);
...
int main(void) {
  _alloc = i_failAlloc_new(&_heap, g_heap, common_HEAPSIZE, -1); /* never fail */
